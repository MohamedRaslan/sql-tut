...
SELECT 'hello, sql' = 'Hello, SQL'; -- case insensitive
...
SELECT 'a' = 'ä'; -- accent insensitive

SELECT 'a' LIKE 'ä';

SELECT 'a' = 'ä' COLLATE utf8mb4_german2_ci; -- UFT, german characters and case insensitive 
...
SELECT ADDDATE('2022-12-08', 24), DAY('2022-12-08'), DAYNAME('2022-12-08'), MONTH('2022-12-08'), MONTHNAME('2022-12-08');
...
-- union : it combine or concatenate rows
...
SELECT *, locate("drama", description)
FROM film_text
WHERE
...
PREPARE sql_stmt
FROM 'SELECT * FROM film_text WHERE title LIKE "Ar%" and film_id > ?';
...
PREPARE sql_stmt
FROM 'SELECT * FROM film_text WHERE title LIKE ? and film_id > ?';
...
-- group by 
-- used mostly with the SQL Aggregate Functions (https://www.w3schools.com/sql/sql_aggregate_functions.asp )
--
SELECT * FROM rental;
...
SELECT customer_id, sum(amount) FROM paymentGROUP BYcustomer_id WHERE sum(amount) > 150
ORDER BY sum(amount) DESC;
-- error!
SELECT customer_id, sum(amount)
FROM payment
...
-- joins  - https://www.owox.com/blog/articles/bigquery-join-and-union-difference/
--
...
DESCRIBE sakila.vw_file_genre

SHOW CREATE VIEW sakila.vw_file_genre